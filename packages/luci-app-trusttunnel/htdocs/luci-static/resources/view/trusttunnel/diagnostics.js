...
var DIAG_TEXT = {
	...
	'it has rewritten the DNS of the whole network': _('it has rewritten the DNS of the whole network'),
	'enabled, and will rewrite the DNS of the whole network on start': _('enabled, and will rewrite the DNS of the whole network on start'),
	'This package only needs its binary — the service is separate. Disable it and clean up: /etc/init.d/https-dns-proxy stop; /etc/init.d/https-dns-proxy disable; then remove server=127.0.0.1#5053/5054, noresolv and every doh_* option from the dhcp config and restart dnsmasq.': _('This package only needs its binary — the service is separate. Disable it and clean up: /etc/init.d/https-dns-proxy stop; /etc/init.d/https-dns-proxy disable; then remove server=127.0.0.1#5053/5054, noresolv and every doh_* option from the dhcp config and restart dnsmasq.'),
